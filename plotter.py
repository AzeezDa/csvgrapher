...
from plot_parameters import PlotParameters, PlotType, TickFormat
...
class Plotter():
    ...
    def __get_line_figure(self):
        ...
        figure = self.__add_line_legend(figure)
    ...
    def __add_line_legend(self, figure):
        """# `__add_line_legend`
        ...
        aspect_ratio = float(self.plot_parameters.width)/self.plot_parameters.height
        dy = (self.plot_parameters.font_size + 8) / self.plot_parameters.height
        for i in range(1, len(y_last_distances)):
    ...
    def __get_pie_figure(self):
        ...
        """
        if self.plot_parameters.based_on:
            figure = px.pie(self.dataframe, 
                        x = self.plot_parameters.x_axis,
                        y = self.plot_parameters.based_on_list)
        else:
            dataframe = self.dataframe.loc[self.dataframe[self.plot_parameters.based_on].isin(self.plot_parameters.based_on_list)]
...
def get_tick_format(type: TickFormat) -> str:
    """# `get_tick_format`
    ...
    """
    match type:
        case TickFormat.FULL:
            return "d"
        case TickFormat.REDUCED:
            return ".3"
        case TickFormat.SI:
            return ".3s"
